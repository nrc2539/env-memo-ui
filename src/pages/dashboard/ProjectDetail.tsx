...
export default function ProjectDetail() {
  ...
  const toggleGroupAll = (group: (typeof mockGroups)[number]) => {
  ...
  const totalVars = mockGroups.reduce((s, g) => s + g.variables.length, 0);
  ...
  return (
    <>
      ...
      <div className="flex h-full gap-6 overflow-hidden">
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-between gap-4 pb-4">
            <div>
              <nav className="mb-1 text-sm text-gray-500">
                <Link to="/dashboard/projects" className="hover:text-teal-600">
                  Projects
                </Link>
                <span className="mx-2">/</span>
                <Link to={`/dashboard/projects/${projectId}`}>
                  <span className="text-gray-900">Frontend App</span>
                </Link>
              </nav>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between px-5 transition hover:bg-gray-50">
                      ...
                      >
                        <IconChevronRight className={`text-gray-400 transition-transform ${isOpen ? "rotate-90" : ""}`} size={16} />
        ...
        {/* Side panel */}
        <div
          className={`fixed inset-y-0 right-0 z-40 w-full max-w-md transform bg-white shadow-xl transition-transform lg:static lg:translate-x-0 ${
            panelOpen ? "translate-x-0" : "translate-x-full"
          } ${!panelOpen && selectedVars.length === 0 ? "lg:hidden" : ""}`}
        >
                  ...
                  {selectedVars.map((v) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => copyToClipboard(`${v.key}=${v.value}`)}
