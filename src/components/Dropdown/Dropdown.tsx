  ...
  return (
    <div className={className}>
      ...
      <button
        type="button"
        className={cn(
          "mt-1 grid grid-cols-[1fr_16px] gap-x-1 items-center px-4 py-2.5 h-10 border border-black rounded-md w-full disabled:text-gray-400",
          buttonClassName,
          {
            "border-info": isOpen,
      ...
      >
        ...
        <IconCaretUpFilled
          className={cn("rotate-180 size-4", { "rotate-0": isOpen })}
        />
      </button>
      ...
      {isOpen && (
        <FloatingPortal>
          <ul
            className={cn(
              "relative p-1 rounded-md shadow-level-3 z-10 bg-white max-h-52 overflow-y-auto w-full",
              menuListClassName,
            )}
      ...
      {!!errorMessage && <ErrorMessage className="mt-1" text={errorMessage} />}
    </div>
  );
}
